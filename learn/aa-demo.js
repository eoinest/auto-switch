...
async function show(){
  ...
  $('#step-list').innerHTML=layout.steps.filter(s=>s.id!==4).map(s=>`<li><strong>${esc(s.title)}</strong>${esc(s.text)}</li>`).join('');
  fit();
...
async function start(){
  ...
  $('#bom').innerHTML='<div class="bom-wrap"><table><thead><tr><th>Part</th><th>Installed quantity</th><th>Buy / reuse</th><th>Purchasing notes</th></tr></thead><tbody>'+rows.map(r=>{
    ...
    return `<tr><td>${esc(r.part)}</td><td>${esc(r.quantity_one_servo)}<br>${esc(r.unit)}</td><td>${esc(r.buy_quantity)}<br>${link(amazon,'Amazon')}${amazon&&url?' · ':''}${link(url,amazon?'Reference / fallback':'Supplier')||(!amazon?'Reuse / see notes':'')}</td><td>${esc(r.notes)}</td></tr>`;
