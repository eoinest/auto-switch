...
Decision recorded 2026-09-05: use **four rechargeable AA NiMH cells**, a ready-made regulated 5 V supply, continuous Wi-Fi, and no GPIO-controlled servo power gate. Start with one servo; retain a path to two channels. The Mac mini relay is optional for direct phone control on the same local network.

The [Amazon-first shopping list](aa-demo-shopping.md) and [BOM CSV](../hardware/aa-demo-bom.csv) are now populated. Use the [illustrated breadboard viewer](../learn/aa-demo.html), [one-servo image](../hardware/wiring/aa-demo/breadboard-1-servo.svg), or [two-servo image](../hardware/wiring/aa-demo/breadboard-2-servo.svg). Their wires, hole assignments, and connector ports pass an independent connectivity check. Physical operation remains untested.
...
Pico GP16 (pin 21) --> 1k series resistor ----------> servo 1 signal
Pico GP17 (pin 22) --> 1k series resistor ----------> optional servo 2 signal
```
...
| Part | One / two servos | Purpose / status |
| --- | --- | --- |
| Pico W; existing headered board for bench | 1 / 1 | Headerless option for the final assembly |
| Existing MG90S with original horn/screw | 1 / 2 | Actual geometry and loaded current still to verify |
| Four matched AA NiMH cells | 4 / 4 | Rechargeable AA; not an interchangeable alkaline or lithium profile |
| External NiMH charger | 1 shared | Cells removed for charging |
| [Pololu 1153 holder](https://www.pololu.com/product/1153) | 1 / 1 | Existing selected four-AA holder |
| [Pololu S18V20F5 / 2574](https://www.pololu.com/product/2574) | 1 / 1 | Existing 5 V step-up/step-down regulator; available output depends on battery voltage and cooling |
| [Pololu 2810 physical master](https://www.pololu.com/product/2810) | 1 / 1 | Retain one existing assembled switch, operated by its slider only; ON input unused |
| SCHURTER 0001.2507 2 A time-lag fuse + Littelfuse 01500274Z holder | 1 each / 1 each | Existing selections; validate against measured pack current and wiring |
| 1N5819 diode | 1 / 1 | D1 in Pico supply branch; striped end toward VSYS |
| Panasonic EEUFR1A471 470 µF, 10 V capacitor | 1 / 1 | Starting bulk capacitor across regulated supply; polarity matters |
| 1 kΩ, 1/4 W series resistors | 1 / 2 | One per servo signal |
| RCY disconnect pair, servo extension(s), 22 AWG power wire, heat-shrink | 1 pair, 1 / 2 extensions | Retain keyed connections and strain relief; verify actual polarity |
| Breadboard and logic jumpers; soldered harness for motor power | 1 shared | Exact replacement breadboard layout pending |
...
| Optional R_PWM1 1 kΩ | f40 | f45 |
| Optional J4 | j21, GP17 strip | j40 |
| Optional L4 | j45 | Second servo signal via extension |
...
- Drive one servo at a time initially. Battery input current can exceed output current when boosting voltage; the regulator's headline current rating does not establish margin for the fused holder and harness.
